...
public class OutBillController  implements Serializable {
    ...
    public void createPdfInvoive() throws JRException, IOException{
        JRBeanCollectionDataSource collectionDataSource = new JRBeanCollectionDataSource(lstBillItemEntrys);
    JasperPrint jasperPrint =        JasperFillManager.fillReport("D:\\tem\\GoodReceiveInvoice01.jasper", new HashMap(), collectionDataSource );
    HttpServletResponse httpServerletResponse = (HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse();
    ServletOutputStream servletOutputStream = httpServerletResponse.getOutputStream();
