...
import gov.sp.health.facade.EstimateItemFacade;
...
@ManagedBean
@SessionScoped
public final class EstimateItemController implements Serializable {

    @EJB
    private EstimateItemFacade ejbFacade;
    @EJB
    InstitutionFacade institutionFacade;
    @ManagedProperty(value = "#{sessionController}")
    SessionController sessionController;
    List<EstimateItem> lstItems;
    private EstimateItem current;
    private DataModel<EstimateItem> items = null;
    ...
    public List<EstimateItem> getLstItems() {
        return getFacade().findBySQL("Select d From EstimateItem d WHERE d.retired=false ORDER BY d.name");
    }

    public void setLstItems(List<EstimateItem> lstItems) {
    ...
    public EstimateItem getCurrent() {
        if (current == null) {
            current = new EstimateItem();
        }
        return current;
    }

    public void setCurrent(EstimateItem current) {
        this.current = current;
    }

    private EstimateItemFacade getFacade() {
        return ejbFacade;
    }

    public DataModel<EstimateItem> getItems() {
        if (institution == null) {
            return new ListDataModel<EstimateItem>(getFacade().findBySQL("SELECT u FROM EstimateItem u WHERE u.retired=false ORDER BY u.name"));
        } else {
            return new ListDataModel<EstimateItem>(getFacade().findBySQL("SELECT u FROM EstimateItem u WHERE u.retired=false AND u.institution.id=" + institution.getId() + "  ORDER BY u.name"));
        }
    }
        ...
        if (items == null) {
            if (selectText.equals("")) {
                ...
                if (items.getRowCount() > 0) {
                    items.setRowIndex(0);
                    current = (EstimateItem) items.getRowData();
    ...
    public EstimateItem searchItem(String itemName, boolean createNewIfNotPresent) {
        EstimateItem searchedItem = null;
        items = new ListDataModel(getFacade().findAll("name", itemName, true));
        if (items.getRowCount() > 0) {
            items.setRowIndex(0);
            searchedItem = (EstimateItem) items.getRowData();
        } else if (createNewIfNotPresent) {
            searchedItem = new EstimateItem();
    ...
    public EstimateItemFacade getEjbFacade() {
        return ejbFacade;
    }

    public void setEjbFacade(EstimateItemFacade ejbFacade) {
    ...
    @FacesConverter(forClass = EstimateItem.class)
    public static class EstimateItemControllerConverter implements Converter {

        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            if (value == null || value.length() == 0) {
                return null;
            }
            EstimateItemController controller = (EstimateItemController) facesContext.getApplication().getELResolver().
                    getValue(facesContext.getELContext(), null, "estimateItemController");
        ...
        public String getAsString(FacesContext facesContext, UIComponent component, Object object) {
            ...
            if (object instanceof EstimateItem) {
                EstimateItem o = (EstimateItem) object;
                return getStringKey(o.getId());
            } else {
                throw new IllegalArgumentException("object " + object + " is of type "
                        + object.getClass().getName() + "; expected type: " + EstimateItemController.class.getName());
            }
        }
    }
}
